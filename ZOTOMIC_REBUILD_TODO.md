...
- 2026-09-09 — **"Only N left" on cart + checkout** (commit `edba43e`, deployed + browser-verified). New `GET /api/storefront/stock?store=&ids=` returns `{stock:{id:{stock,tracked}}}` for product/variant ids. Shared `components/storefront/StockLine.tsx` (`useLineStock` hook + `<StockLine>`): renders "Only N left" (≤5), "Out of stock" (≤0), or "Only N available — reduce the quantity" (qty>stock) under each line on the cart page and the checkout order-summary. Checkout disables **Place order** + shows a banner while any line is over/out of stock.
