...
import {
  addOwnerTelegramBot,
  updateOwnerTelegramBot,
  deleteOwnerTelegramBot,
  testOwnerTelegramBot,
  type OwnerTgBotRow,
} from "./actions";

export function OwnerTelegramBots({ bots, webhookUrl }: { bots: OwnerTgBotRow[]; webhookUrl: string }) {
      ...
      <CardHeader>
        <CardTitle>
          <span className="flex items-center gap-2">
            <Send className="h-4 w-4 text-primary" /> Chat on Telegram
          </span>
        </CardTitle>
        <span className="text-xs text-fg-subtle">Talk to your assistant from your phone</span>
      </CardHeader>

      <CardBody className="space-y-4">
        {bots.length === 0 && !adding && (
          <p className="text-sm text-fg-muted">
            Connect a Telegram bot and ask your assistant about sales, stock, orders and customers from anywhere.
            Create one with <b>@BotFather</b>, then add its token here. Assistant credits are used the same as on the web.
          </p>
        )}
          ...
          <ul className="divide-y divide-border rounded-lg border border-border">
            {bots.map((b) => (
              <li key={b.id} className="flex flex-wrap items-center gap-3 px-3 py-2.5 text-sm">
                ...
                <button
                  onClick={() => run(() => testOwnerTelegramBot(b.id), "Test sent")}
                ...
                <button
                  onClick={() => run(() => updateOwnerTelegramBot(b.id, { enabled: !b.enabled }), "Updated")}
                  className="text-fg-subtle hover:text-fg"
                ...
                <button
                  onClick={() => {
                    if (confirm(`Remove "${b.label}"? Its Telegram webhook will be deleted.`))
                      run(() => deleteOwnerTelegramBot(b.id), "Removed");
                  }}
        ...
        {adding ? (
          <div className="space-y-3 rounded-lg border border-dashed border-border p-3">
            ...
            <p className="text-xs text-fg-subtle">
              Message your bot once first (send it any text) so Telegram lets it reply. Webhook: <code>{webhookUrl}</code>
            </p>
            <div className="flex justify-end gap-2">
              ...
              <Button
                ...
                onClick={() =>
                  run(async () => {
                    const res = await addOwnerTelegramBot({ label, token, chatId });
                    if ("ok" in res) {
                      setAdding(false);
                      setLabel("");
                      setToken("");
