      ...
      <Card>
        ...
        <CardBody>
          <NotificationMatrix
            ...
            note={telegramLinked ? "Telegram alerts go to the chat ID set above." : "Add a Telegram chat ID above to enable Telegram alerts."}
          />
        </CardBody>
      </Card>
    );
        ...
        <form action={save} className="space-y-4">
          ...
          <div className="border-t border-border pt-4">
            ...
            <div className="space-y-4">
              ...
              {brandedInvoice ? (
                <Field label="Invoice reply-to email" hint="Customer replies to invoices go here. Invoices still send from Zotomic's mail server.">
                  <Input name="invoice_from_email" type="email" defaultValue={business.invoice_from_email ?? ""} placeholder="orders@yourstore.com" />
                </Field>
              ) : (
                <p className="rounded-sm border border-border bg-surface-2 px-3 py-2 text-xs text-fg-subtle">
                  On a paid plan you can set your own reply-to address for customer invoices, and drop the &ldquo;Powered by
                  Zotomic&rdquo; footer.
                </p>
              )}
            </div>
          </div>
