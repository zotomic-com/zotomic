...
export default async function ProductsPage() {
  ...
  const [{ data }, { data: variants }, { data: cats }, { data: soldRows }] = await Promise.all([
    db
      .from("products")
      .select(
        "id, name, sku, category, status, price, sale_price, buying_price, marketing_cost, stock_qty, track_inventory, image_urls, options, has_variants, is_hot, hide_badges",
      )
    ...
    db.from("order_items").select("product_id, qty").eq("business_id", tenant.businessId),
  ]);

  const variantsByProduct: Record<string, unknown[]> = {};
  for (const v of variants ?? []) (variantsByProduct[v.product_id as string] ??= []).push(v);

  const soldByProduct = new Map<string, number>();
  for (const r of soldRows ?? [])
    soldByProduct.set(r.product_id as string, (soldByProduct.get(r.product_id as string) ?? 0) + Number(r.qty));
  ...
  const products = (data ?? []).map((p) => ({
    ...
    sold: soldByProduct.get(p.id as string) ?? 0,
  })) as ProductRow[];
