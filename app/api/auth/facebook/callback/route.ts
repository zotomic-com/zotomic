...
export async function GET(req: NextRequest) {
  ...
  const profile = await exchangeFacebookCode(code, new URL("/api/auth/facebook/callback", origin).toString());
  if ("error" in profile) return fail(profile.error);

  const result = await completeOAuthSignIn({ provider: "facebook", providerId: profile.id, email: profile.email, name: profile.name });
