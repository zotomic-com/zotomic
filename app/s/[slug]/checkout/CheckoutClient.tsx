...
export function CheckoutClient({
  ...
  const field = "h-11 w-full rounded-[var(--sf-radius)] border border-[var(--sf-line)] bg-[var(--sf-bg)] px-3.5 text-sm outline-none focus:border-[var(--sf-accent)]";

  return (
    <form onSubmit={submit} className="grid gap-8 lg:grid-cols-[1fr_340px]">
      <div className="space-y-4">
        <div>
          <p className="mb-2 text-sm font-bold">Delivery details</p>
          <div className="space-y-3">
            ...
            <textarea required placeholder="Delivery address" rows={3} className={`${field} h-auto py-2.5`} value={form.address} onChange={(e) => setForm((f) => ({ ...f, address: e.target.value }))} />
            <input placeholder="City / area" className={field} value={form.city} onChange={(e) => setForm((f) => ({ ...f, city: e.target.value }))} />
            <textarea placeholder="Order note (optional)" rows={2} className={`${field} h-auto py-2.5`} value={form.note} onChange={(e) => setForm((f) => ({ ...f, note: e.target.value }))} />
          </div>
        </div>
