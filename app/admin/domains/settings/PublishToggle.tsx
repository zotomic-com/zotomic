...
export function PublishToggle({ enabled }: { enabled: boolean }) {
  ...
  const toggle = () =>
    start(async () => {
      ...
      fd.set("domain_reseller_enabled", enabled ? "false" : "true");
      await saveWebsiteSettings(fd);
      toast(enabled ? "Unpublished" : "Published — /domains is live", "success");
      router.refresh();
    });
  ...
  return (
    <label className="flex cursor-pointer items-center gap-3 text-sm">
      ...
      <span className="font-medium text-fg">{pending ? "Saving…" : enabled ? "Published — live now" : "Unpublished"}</span>
    </label>
  );
}
